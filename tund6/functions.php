<?php
	...
	function listIdeas(){
		$notice ="";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUserName"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT idea, ideaColour FROM vpuserIdeas WHERE userID = ? ORDER BY created DESC");
		echo $mysqli->error;
		
		$stmt->bind_param("s", $_SESSION["userID"]);
		$stmt->bind_result($idea, $colour);
		$stmt->execute();
		
		while($stmt->fetch()){
			$notice .= "<p style='background-color:". $colour ."' >". $idea ."</p> \n";
		}
